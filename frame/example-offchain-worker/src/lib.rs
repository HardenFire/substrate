...
//!
//! Run `cargo doc --package pallet-example-offchain-worker --open` to view this pallet's
//! documentation.
//!
...
/// Defines application identifier for crypto keys of this pallet.
///
/// Every pallet that deals with signatures needs to declare its unique identifier for
/// its crypto keys.
...
mod pallet {
	...
	pub(crate) type Prices<T> = StorageValue<_, Vec<u32>, ValueQuery>;
	...
	pub(crate) type NextUnsignedAt<T: Config> = StorageValue<_, T::BlockNumber, ValueQuery>;

	/// Events generated by the pallet.
	#[pallet::event]
	...
	pub enum Event<T: Config> {
		...
		/// \[price, who\]
		NewPrice(u32, <T as frame_system::Config>::AccountId),
	}
	...
	impl<T: Config> Pallet<T> {
		...
		///
		/// This method is a public function of the pallet and can be called from within
...
/// Most of the functions are moved outside of the `pallet` macro.
///
/// This helps with error messages, as the ones inside the macro can sometimes be hard to debug.
impl<T: Config> Pallet<T> {
	...
	fn choose_transaction_type(block_number: T::BlockNumber) -> TransactionType {
		...
		// to prepend your entry with the pallet name.
